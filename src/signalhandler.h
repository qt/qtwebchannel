 ...
 *
 * It allows to connect to any number of signals of arbitrary objects and forwards the signal
...
private:
    ...
    ConnectionHash m_connectionCounter;
};
...
{
    ConnectionPair &connectionCounter = m_connectionCounter[object][signalIndex];
...
{
    Q_ASSERT(m_connectionCounter.value(object).contains(signalIndex));
    ConnectionPair &connection = m_connectionCounter[object][signalIndex];
    --connection.second;
    if (!connection.second || !connection.first) {
        QObject::disconnect(connection.first);
        m_connectionCounter[object].remove(signalIndex);
        if (m_connectionCounter[object].isEmpty()) {
            m_connectionCounter.remove(object);
        }
    }
}
    ...
    if (call == QMetaObject::InvokeMetaMethod) {
        if (methodId == 0) {
            ...
            if (senderSignalIndex() == destroyedIndex) {
                ConnectionHash::iterator it = m_connectionCounter.find(sender());
                Q_ASSERT(it != m_connectionCounter.end());
                foreach (const ConnectionPair &connection, *it) {
                    QObject::disconnect(connection.first);
                }
                m_connectionCounter.erase(it);
            }
        }
        --methodId;
    }
...
{
    foreach (const SignalConnectionHash &connections, m_connectionCounter) {
        foreach (const ConnectionPair &connection, connections) {
            QObject::disconnect(connection.first);
        }
    }
    m_connectionCounter.clear();
