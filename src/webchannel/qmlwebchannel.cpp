...
{
    return d_func()->publisher->clientIsIdle;
}
...
void QmlWebChannel::objectIdChanged(const QString &newId)
{
    ...
    Q_ASSERT(d->registeredObjects.contains(attached->parent()));
...
{
    QmlWebChannel *channel = static_cast<QmlWebChannel*>(prop->object);
    return dynamic_cast<QObject*>(channel->d_func()->transports.at(index));
}
