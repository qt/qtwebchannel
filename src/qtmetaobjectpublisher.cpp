...
static const QString KEY_DESTROYED = QStringLiteral("destroyed");
static const QString KEY_SIGNAL = QStringLiteral("signal");
static const QString KEY_TYPE = QStringLiteral("type");
static const QString KEY_MESSAGE = QStringLiteral("message");
static const QString KEY_METHOD = QStringLiteral("method");
static const QString KEY_ARGS = QStringLiteral("args");
static const QString KEY_PROPERTY = QStringLiteral("property");
static const QString KEY_VALUE = QStringLiteral("value");

static const QString TYPE_SIGNAL = QStringLiteral("Qt.signal");
static const QString TYPE_PROPERTY_UPDATE = QStringLiteral("Qt.propertyUpdate");
static const QString TYPE_INIT = QStringLiteral("Qt.init");
static const QString TYPE_IDLE = QStringLiteral("Qt.idle");
static const QString TYPE_DEBUG = QStringLiteral("Qt.Debug");
static const QString TYPE_INVOKE_METHOD = QStringLiteral("Qt.invokeMethod");
static const QString TYPE_CONNECT_TO_SIGNAL = QStringLiteral("Qt.connectToSignal");
static const QString TYPE_DISCONNECT_FROM_SIGNAL = QStringLiteral("Qt.disconnectFromSignal");
static const QString TYPE_SET_PROPERTY = QStringLiteral("Qt.setProperty");

static QString objectId(const QObject *object)
{
...
struct QtMetaObjectPublisherPrivate
{
     ...
     *
     * TODO: support wrapping of innitially registered objects
     */
                                                ...
                                                const QJsonArray &args, const QJsonValue &id)
{
    ...
    if (method.name() == QByteArrayLiteral("deleteLater")) {
        ...
        qWarning() << "Cannot invoke method" << method.name() << "on object" << object << "with more than 10 arguments.";
        return false;
    } else if (args.size() > method.parameterCount()) {
        qWarning() << "Cannot invoke method" << method.name() << "on object" << object << "with" << args.size() << "arguments, it takes only " << method.parameterCount();
        return false;
    }
...
{
    connect(&d->timer, SIGNAL(timeout()), SLOT(sendPendingPropertyUpdates()));
}
...
void QtMetaObjectPublisher::bench_registerObects(const QVariantMap &objects)
{
    d->propertyUpdatesInitialized = false;
...
#include "moc_qtmetaobjectpublisher.cpp"
